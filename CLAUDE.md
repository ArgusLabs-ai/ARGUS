...
6. **LLM semantic judge** (`semantic_checker.py`): on by default when a key is configured. Returns a `failure_kind`; only `unrelated` / `contradiction` (semantic coherence — nothing else can detect it) may fail a step with no deterministic layer agreeing, and such a verdict is re-asked once and must reproduce (the second look *audits the first verdict* rather than replaying the same prompt, which reproduced its own misreads at temperature 0). `empty_or_missing` / `other` need corroboration, and only a **critical** rule finding corroborates — a warning is the rules saying "noted, not a failure" (`empty_result` on a scanner that found nothing), so letting the judge upgrade one let it override a deliberate calibration. **The judge may not originate blame downstream of an existing origin** (`_judge_may_fail_this_step` / `_demote_consequential_judge_fails`): a node fed a bad answer restates it, and the judge calls that a contradiction — of the consequence, not the cause. Enforced again at finalize, because a per-step verdict is computed before the contextual layer has blamed anyone. For the same reason the verdict is applied only if nothing else claimed the step meanwhile: writing it unconditionally *erased* contextual blame, so the origin came back `pass` while its victim was flagged. A non-gating verdict is still recorded, as a `warning` finding. Evidence-aware final ruling — receives all prior signals (validator results, anomaly signals, inspection findings) as context, plus the ledger rows *before* this step (`prior_rows`, #85) scoped to the fields the node reads or writes, so an upstream emptying is visible instead of only "this field is empty now". Cannot override validator failures or critical anomalies. Returns `evidence_considered` and `overridden_signals` for audit trail.
