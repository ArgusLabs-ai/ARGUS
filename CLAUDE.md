...
7. Status assigned: `pass | fail | crashed | semantic_fail | degraded_input | interrupted` (plus `retried` / `skipped` set at finalize — `retried` is per **round**: `Send` siblings share a `NodeEvent.superstep` and never relabel each other, E4; an earlier iteration that wrote a data accumulator (`operator.add`, kind `"add"`) is not relabelled, while `add_messages` still is, E4b / #131). Full vocabulary and the node → run roll-up: `docs/STATUS.md`
